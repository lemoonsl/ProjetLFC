...
<LIGNEOK>DTSTART: {inserer_symbole("EVT", "UNIQ", NULL, NULL);inserer_symbole("DAT", "DEBUT", 0, yyleng); printf("intro heure début evt unique\n"); BEGIN DATEUOK;return IDEBEVTU;}
<LIGNEOK>DTEND: {inserer_symbole("DAT", "FIN", 0, yyleng); printf("intro heure fin evt unique\n"); BEGIN DATEUOK;return IFINEVTU;}
...
<LIGNEOK>DTSTART;TZID=[a-zA-Z/]+: {inserer_symbole("EVT", "REPET", NULL, NULL);inserer_symbole("DAT", "DEBUT", 0, yyleng); printf("intro heure début evt répétitif\n");
BEGIN DATEROK;return DEBEVTR;}
<LIGNEOK>DTEND;TZID=[a-zA-Z/]+: {inserer_symbole("DAT", "FIN", 0, yyleng); printf("intro heure fin evt répétitif\n");
BEGIN DATEROK;return FINEVTR;}
<LIGNEOK>DTSTART;VALUE=DATE: {inserer_symbole("EVT", "JOUR", NULL, NULL);inserer_symbole("DAT", "DEBUT", 0, yyleng); printf("intro heure début evt journée\n");
BEGIN DATEJOK;return DEBEVTJ;}
<LIGNEOK>DTEND;VALUE=DATE: {inserer_symbole("DAT", "FIN", 0, yyleng); printf("intro heure fin evt journée\n");
BEGIN DATEJOK;return FINEVTJ; }
<ALARM>"-P"[0-9]+DT[0-9]+H[0-9]+M[0-9]+S {printf("position alarme : %s\n", yytext);return POSAL;}
<DATEJOK>[0-9]{8} {ajouter_chaine_tableau2(yytext);printf("date evt journée : %s\n", yytext); BEGIN LIGNEOK;return DATEVTJ; }
